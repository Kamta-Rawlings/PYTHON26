...
def test_infix_private() -> None:
    exp = InfixExpression("a")
    assert ("_expression" in dir(exp) and exp.expression == exp._expression) or \
        ("_InfixExpression__expression" in dir(exp) and \
         exp.expression == exp._InfixExpression__expression)
